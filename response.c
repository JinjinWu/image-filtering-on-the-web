...
void image_filter_response(int fd, const ReqData *reqData) {
    ...
    // set initial flag values to where params are
    int index_image = -1;
    int index_filter = -1;

    // find indices of which params have the image name and filter name
    for (int i = 0; i < MAX_QUERY_PARAMS && reqData->params[i].name != NULL; i++) {
	if (strcmp(reqData->params[i].name, "image") == 0) {
	    index_image = i;
	} else if (strcmp(reqData->params[i].name, "filter") == 0) {
	    index_filter = i;
	}
    }

    // the relevant parameters don't exist
    if (index_image == -1 || index_filter == -1) {
	bad_request_response(fd, "bad request error");
	exit(1);
    }

    char *image_name = reqData->params[index_image].value;
    char *filter_name = reqData->params[index_filter].value;

    for (int i = 0; i < strlen(image_name); i++) {
	if (image_name[i] == '/') {
	    bad_request_response(fd, "bad request error");
	    exit(1);
	}
    ...
    // create and concatenate image_path string
    char image_path[30 + strlen(image_name)];
    memset(image_path, '\0', 30 + strlen(image_name));
    strncpy(image_path, "./", 2);
    strncat(image_path, IMAGE_DIR, 27);
    strncat(image_path, image_name, strlen(image_name));

    // create and concatenate filter_path string
    char filter_path[30 + strlen(filter_name)];
    memset(filter_path, '\0', 30 + strlen(filter_name));
    strncpy(filter_path, "./", 2);
    strncat(filter_path, FILTER_DIR, 27);
    strncat(filter_path, filter_name, strlen(filter_name));

    // check if image and filter exist and can be executed/read
    if (access(image_path, F_OK) == -1 || access(image_path, R_OK) == -1 ||
	access(filter_path, F_OK) == -1 || access(filter_path, X_OK) == -1) {
	bad_request_response(fd, "bad request error");
	exit(1);
    }
    ...
    execl(filter_path, reqData->params[index_filter].value, NULL);
